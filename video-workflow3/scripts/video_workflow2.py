...
def main(
    ...
    Args:
        ...
        director: 导演台本/分段台本（可选，与topic/script二选一）
...
if __name__ == "__main__":
  ...
  3. 通过导演/分段直接生成视频: python video_workflow.py -i "C:\\1.png" -d '["片段1","片段2"]'
    ...
    parser.add_argument(
        '-d', '--director',
        help='导演台本/分段台本（JSON 数组如 ["片段1","片段2"]，或多行/用|分隔的片段；与--topic/--script二选一）',
    )
    parser.add_argument(
        '--end-step',
        dest="end_step",
    ...
    if topic_provided:
        ...
        print(f"文案脚本: {res}")
        print("停止工具调用和思考，将文案脚本（script）输出给用户，让其决定下一步执行")
    elif script_provided:
        res = {"segments": result.get("segments", "生成为空，可能出现错误。")}
        print(f"导演台本: {res}")
        print("停止工具调用和思考，将导演台本（segments）输出给用户，让其决定下一步执行")
