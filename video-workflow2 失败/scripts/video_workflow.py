...
def build_workflow():
    ...
    # 节点间固定流程
    graph.add_edge("generate_script", "split_script")
    graph.add_edge("split_script", "generate_videos")
...
# ====================== 7. 输入验证（改进3） ======================
def validate_inputs(topic: str = None, script: str = None, reference_image: str = None) -> bool:
    """
    验证输入参数（改进3：入口处检查）
    ...
    if not topic and not script:
        errors.append("必须提供 topic（主题）或 script（文案）之一")
...
# ====================== 8. 主程序入口 ======================
def main(topic: str = None, script: str = None, reference_image: str = None):
    """
    主函数
    ...
    if not validate_inputs(topic=topic, script=script, reference_image=reference_image):
        print("\n❌ 工作流终止，请检查输入参数")
        return None
    ...
    if topic:
        ...
        print(f"📌 模式: 从主题生成文案 (topic={topic})")
    else:
...
# 创建参数解析器
  ...
  2. 通过文案生成: python video_workflow.py -i "C:\\1.png" -s "视频生成技术已经悄悄进入人们的生活....."
    ...
    parser.add_argument(
        '-t', '--topic', 
        help='生成视频的主题 (与--script二选一)'
    )
    parser.add_argument(
        '-s', '--script',
        help='视频文案内容 (与--topic二选一)'
    )
    ...
    if topic_provided and script_provided:
        print("错误: 不能同时提供--topic(-t)和--script(-s)参数！", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
    
    if not topic_provided and not script_provided:
        print("错误: 必须提供--topic(-t)或--script(-s)其中一个参数！", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
