---
name: video-workflow
description: AI数字人视频生成工作流 - 根据口播主题/文案调用Seedance生成视频片段并合并。提供命令行调用方式。
...
AI视频生成工作流技能，根据口播主题或文案自动生成视频。该技能为交互式执行，你需要通过对话记忆来选择执行命令。
...
|------|--------|------|------|
| --img | -i | 参考图片路径 | 是 |
| --topic | -t | 根据主题生成口播文案 | 否 |
| --script | -s | 根据口播文案生成导演脚本 | 否 |
| --director | -d | 根据导演脚本生成视频 | 否 |
